...
void main() {
  ...
  if (sample_location.x > 0.0) {
    border_radius = (sample_location.y < 0.0) ? in_radius.y : in_radius.z;
  } else {
    border_radius = (sample_location.y < 0.0) ? in_radius.x : in_radius.w;
  }

  float outer_dist =
      sdf_rounded_rect(sample_location, half_size, border_radius);
  ...
  float inner_dist =
      sdf_rounded_rect(sample_location, inner_half_size, border_radius);

  float outer_alpha = 1.0 - smoothstep(-1, 1.0, outer_dist);
  float inner_alpha = 1.0 - smoothstep(-1, 1.0, inner_dist);
